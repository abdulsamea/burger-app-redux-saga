...
export function* logoutSaga(action) {
    yield localStorage.removeItem('burger-token');
...
export function* authUserSaga(action) {
        ...
        yield localStorage.setItem('burger-token', response.data.idToken);
...
export function* authCheckStateSaga(action) {
        const token = yield localStorage.getItem('burger-token');
        if(!token) {
            yield put(actions.logout());
        }
        else{
